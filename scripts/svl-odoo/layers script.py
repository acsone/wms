...
# Script obtained from Odoo Support under the following conditions 

# Cependant, une fois de plus, je tiens à insister sur le fait que ce script ne doit
# être partagé avec personne et ne doit être utilisé dans aucun autre cas. Nous ne
# serons pas tenus responsables des dommages causés par son utilisation.

# Dans le script lui-même, il y a plusieurs modes que vous pouvez choisir d'exécuter ou
# de ne pas exécuter. Je ne changerais aucun des paramètres par défaut pour vous et
# commencerais par l'exécuter sans valider quoi que ce soit en utilisant une dizaine de
# produits environ, juste pour voir le rapport et les résultats potentiels (introduire
# les id dans le parametre specific_products). Si vous êtes satisfait du rapport,
# essayez ensuite de valider sur ladite liste d'une dizaine de produits ou quelque chose
# du genre.
...
log_context_header = "{}{}{}".format("By " + trigram + " " if trigram else "", "For " + ticket_ref if ticket_ref else "", "\n\n" if trigram or ticket_ref else "")
...
def restore_product_valuation(product):
    ...
    _write_product_report("Initial " + _VALUATION_MESSAGE.format(valuation_initial['quantity'], valuation_initial['value'], p=precs), product, 1, 0)
    _write_product_report("Final " + _VALUATION_MESSAGE.format(product.quantity_svl, product.value_svl, p=precs), product, 0, 1)
...
def _init_product_restoration(product):
    ...
    if product_db_data['state'] == STATE_NEW:
        ...
        write_product_to_db(product, STATE_IN_PROGRESS, product_reports[product.id], valuation_total, valuation_initial, [], None, None)
        commit()
    ...
    if product_db_data['state'] != STATE_DONE:  # Operation can be slow and is not needed when product is Done
        stock_moves = get_product_stock_moves(product, product_db_data['current_stock_move_id'])
...
def get_products(company=None):
    ...
    env.cr.execute("SELECT product_id FROM tech_support_fix_valuation_v2 WHERE state = 1 AND company_id = %s", [company.id])
...
def update_with_misc_valuation(valuation_total, product, date_start=None, date_end=None):
    ...
    handled_price_changes = {  # key: description to match, value: get new_unit_cost function
        'Product value manually modified': lambda description: float(description.split()[-1][:-1]),  # format: 'Product value manually modified (from <old_uc> to <new_uc>)'
        'Product cost updated': lambda description: float(description.split()[-1][:-1]),  # format: 'Manual Stock Valuation: <custom_comment>. Product cost updated from <old_uc> to <new_uc>.'
    ...
    for m_svl in StockValuationLayer.search(misc_domain):
        ...
        current_valuation = get_current_valuation(m_svl.product_id)
        value_delta = m_svl.currency_id.round(current_valuation['quantity'] * new_unit_cost - current_valuation['value'])  # Correct logic

        if float_compare(value_delta, m_svl.value, precision_rounding=m_svl.currency_id.rounding) != 0:
            new_description = 'Product value manually modified (from {} to {})'.format(product.standard_price, new_unit_cost)
            _write_product_report("\t - Misc SVL value updated : from {:.2f} to {} -> {}\n".format(m_svl.value, value_delta, new_description), m_svl.product_id)
    ...
    if misc_value_total != 0:
        valuation_total.update({'value': valuation_total['value'] + misc_value_total})
        _write_product_report("\tNew Current " + _VALUATION_MESSAGE.format(valuation_total['quantity'], valuation_total['value'], p=precs), product)
...
def restore_stock_move_valuation(stock_move, valuation_total):
    ...
    _write_product_report(_STOCK_MOVE_MESSAGE.format(direction=get_move_type(stock_move).upper(), sm=stock_move, p=precs), stock_move.product_id, 1, 0)
    ...
    if not valued_quantity:
        ...
        _write_product_report("\tStock Move #{} has no quantity, continue.\n".format(stock_move.id), stock_move.product_id)
        if svl.exists():
    ...
    _write_product_report("\tCurrent " + _VALUATION_MESSAGE.format(valuation_total['quantity'], valuation_total['value'], p=precs), stock_move.product_id)
...
def get_product_stock_moves(product, sm_id=None):
    """
    ...
    stock_moves = StockMove.search([
        ('product_id', '=', product.id),
        ('company_id', '=', env.company.id)
    ], order='date ASC, id ASC').filtered(
        lambda sm: sm.state == 'done' and get_move_type(sm) is not None
    )
...
def _write_svl_update_report(svl, new_quantity, new_unit_cost, new_value):
    if float_compare(svl.quantity, new_quantity, precision_digits=precs['qp']) != 0:
        _write_product_report("\tQuantity changed from {:.{p[qp]}f} to {:.{p[qp]}f}\n".format(svl.quantity, new_quantity, p=precs), svl.product_id)
    if float_compare(svl.unit_cost, new_unit_cost, precision_digits=precs['cp']) != 0:
        _write_product_report("\tUnit Cost changed from {:.{p[cp]}f} to {:.{p[cp]}f}\n".format(svl.unit_cost, new_unit_cost, p=precs), svl.product_id)
    if float_compare(svl.value, new_value, precision_digits=precs['vp']) != 0:
        _write_product_report("\tValue changed from {:.{p[vp]}f} to {:.{p[vp]}f}\n".format(svl.value, new_value, p=precs), svl.product_id)
...
def compute_dropship_svls(stock_move, svls, valued_quantity):
    ...
    """
    if len(svls) not in [0, 2] or\
            (len(svls) == 2 and any(is_zero(q, digits=precs['qp']) for q in svls.mapped('quantity'))) or\
            (len(svls) == 2 and not is_zero(sum(svls.mapped('quantity')), digits=precs['qp'])):
        _write_product_report("\tDropship SVLs incorrect: svls #{} unlinked.\n".format(len(svls), ), stock_move.product_id)
        svls.sudo().unlink()
    ...
    _write_product_report(("\tNew " if is_new_svls else "\t") + _SVL_MESSAGE.format(svl=in_svl, uc_src=','.join(uc_src), p=precs), stock_move.product_id)
    _write_product_report(("\tNew " if is_new_svls else "\t") + _SVL_MESSAGE.format(svl=out_svl, uc_src=','.join(uc_src), p=precs), stock_move.product_id)
    return svls
...
def compute_svl(stock_move, svl, valued_quantity):
    ...
    if is_new_svl := not svl.exists():  # Create new SVL
        svl = create_svls({
            ...
            'company_id': env.company.id,
            'description': stock_move.reference and '%s - %s' % (stock_move.reference, stock_move.product_id.name) or stock_move.product_id.name,
    ...
    _write_product_report(("\tNew " if is_new_svl else "\t") + _SVL_MESSAGE.format(svl=svl, uc_src=','.join(uc_src), p=precs), stock_move.product_id)
    return svl
...
def compute_landed_cost(svl):
    ...
    """
    if not model_exists('stock.valuation.adjustment.lines') or svl.product_id.cost_method == 'standard' or not svl.stock_move_id._is_in():
        return 0
    ...
    for cost in cost_adjustment_lines:
        ...
        _write_product_report("\t\tNew " + _IN_REM_DATA_MESSAGE.format(svl=svl, rem_uc=get_remaining_unit_cost(svl), p=precs), svl.product_id)
...
def clean_svl(product):
    ...
    """
    svls = StockValuationLayer.search([('company_id', '=', env.company.id), ('product_id', '=', product.id)])
    if delete_before:
    ...
    for svl in svls:
        ...
        elif not svl.stock_move_id and not svl.stock_valuation_layer_id and ('Costing method change' in svl.description or 'Valuation method change' in svl.description):
            deletion_reason = "Costing/Valuation method change"
        elif not svl.stock_move_id and not svl.stock_valuation_layer_id and not any(svl_desc in any_data['svl.description'] for svl_desc in misc_svls_descriptions):
            deletion_reason = "Unknown misc SVL: {}".format(svl.description)
        elif not svl.stock_move_id and svl.stock_valuation_layer_id and odoo_version >= 16 and svl.account_move_line_id:  # field account_move_line_id does not exist before Odoo 16
...
def create_svls(data):
    ...
    if 'create_date' in data[0]:  # Set correct create_date
        env.cr.execute("UPDATE stock_valuation_layer SET create_date = %s, write_date = %s WHERE id IN %s;", [data[0]['create_date'], data[0]['create_date'], tuple(svls.ids)])
...
def revaluate_negative_valuation(in_svl):
    ...
    for prev_out_svl in previous_out_svls:
        ...
        if not is_zero(value_delta, rounding=in_svl.currency_id.rounding):
            ...
            _write_product_report("\t\t" + _REVALUATION_MESSAGE.format(prev_out_svl.id, quantity_to_revaluate, value_delta, p=precs), in_svl.product_id)
    ...
    _write_product_report("\t\tNew " + _IN_REM_DATA_MESSAGE.format(svl=in_svl, rem_uc=get_remaining_unit_cost(in_svl), p=precs), in_svl.product_id)
...
def _compute_out_remaining_data(svl):
    ...
    for svl_in in previous_in_svls:
        ...
        _write_product_report("\t\tNew " + _IN_REM_DATA_MESSAGE.format(svl=svl_in, rem_uc=get_remaining_unit_cost(svl_in), p=precs), svl.product_id)
...
def compute_remaining_data(svl):
    ...
    if svl.stock_move_id._is_in():
        ...
        _write_product_report("\t\t" + _IN_REM_DATA_MESSAGE.format(svl=svl, rem_uc=get_remaining_unit_cost(svl), p=precs), svl.product_id)
...
def compute_standard_price(product, svl):
    ...
    if svl.stock_move_id._is_dropshipped() or svl.stock_move_id._is_dropshipped_returned():
        ...
        return
    if svl.stock_move_id._is_out():
        _write_product_report("\tCompute Standard Price should not be called for OUT moves. If not done intentionally, please investigate.", product)
    ...
    if product.cost_method == 'average':
        if quantity_svl > 0:
            ...
            oldest_out_svl = StockValuationLayer.search(svl_domain + [('remaining_qty', '<', 0)], order='create_date asc, id asc', limit=1)
            new_std_price = oldest_out_svl.unit_cost

    elif product.cost_method == 'fifo':
        if quantity_svl > 0:
            # Get uc from oldest IN svl with rem qty
            oldest_in_svl = StockValuationLayer.search(svl_domain + [('remaining_qty', '>', 0)], order='create_date asc, id asc', limit=1)
            ...
            last_in_svl = StockValuationLayer.search(svl_domain + [('quantity', '>', 0)], order='create_date desc, id desc', limit=1)
            ...
            new_std_price = last_in_value / last_in_quantity  # Because remaining_qty is 0, we recompute the unit_cost using the revaluation svls linked to the last_in svl
...
def _compute_out_avco_uc_and_value(svl):  # OUT average
    ...
    if float_compare(quantity_svl, 0, precision_digits=precs['qp']) == 1:
        ...
        max_allowed_rounding_error = currency.round(currency.rounding * abs(svl.quantity) / 2 + currency.rounding * 2)  # Check more lenient than standard
        if rounding_error and abs(rounding_error) <= max_allowed_rounding_error:
            ...
            _write_product_report("/!\\ Rounding Error detected and not adjusted: {} > max {}\n".format(rounding_error, max_allowed_rounding_error), svl.product_id)
...
def _compute_in_avco_uc_and_value(svl):  # IN average/fifo
    ...
    def _get_from_purchase_line_id(stock_move):
        ...
        price_unit = abs(line.currency_id._convert(line.price_unit, env.company.currency_id, env.company, stock_move.date, round=False))
    ...
    def _get_from_purchase_kit(stock_move):
        line = stock_move.purchase_line_id or stock_move.move_dest_ids.purchase_line_id[:1]
        if not line or is_zero(line.price_unit) or stock_move.product_id == line.product_id or not stock_move.bom_line_id:
            return None

        # Copy/paste from _get_from_purchase_line_id()
        kit_price_unit = abs(line.currency_id._convert(line.price_unit, env.company.currency_id, env.company, stock_move.date, round=False))
        ...
        uc = float_round(kit_price_unit * cost_share * line.product_qty / stock_move.product_qty, precision_digits=precs['cp'])
    ...
    def _get_from_vendor_bill(stock_move):
        ...
        for aml in stock_move.purchase_line_id.invoice_lines:
            aml_uom_qty = aml.product_uom_id._compute_quantity(aml.quantity, stock_move.product_id.uom_id, rounding_method='HALF-UP')
            if aml.balance != 0 and is_zero(aml_uom_qty - stock_move.product_uom_qty, rounding=stock_move.product_id.uom_id.rounding):
    ...
    def _get_from_origin(stock_move):
        if stock_move.origin_returned_move_id and stock_move.origin_returned_move_id.stock_valuation_layer_ids:
            ...
            if stock_move.origin_returned_move_id._is_dropshipped() or stock_move.origin_returned_move_id._is_dropshipped_returned():
                layers = layers.filtered(lambda lay: float_compare(lay.value, 0, precision_rounding=lay.product_id.uom_id.rounding) <= 0)
...
def compute_unit_cost_and_value(svl):
    ...
    compute_methods = {
        ...
        ('in_average', 'in_fifo', 'dropship_average', 'dropship_fifo', 'dropship_return_average', 'dropship_return_fifo'): _compute_in_avco_uc_and_value,
...
def save_progress(product, stock_move, valuation_total, valuation_initial, last_svl_date, svls_ids):
    ...
    if time.time() - timings["start"] >= timings["after_start"] and time.time() - timings["last_save"] >= timings["after_save"]:
        write_product_to_db(product, STATE_IN_PROGRESS, product_reports[product.id], valuation_total, valuation_initial, svls_ids, stock_move.id, last_svl_date)
        commit()
...
def _get_product_report(product):
    if product.id not in product_reports:
        return "No report for product {}.".format(product.name)

    return product_reports[product.id]
...
def write_product_to_db(product, state, report, tt_val, init_val, processed_svls, sm_id, last_svl_date):
    ...
    if len(env.cr.fetchall()):
        ...
        query = "INSERT INTO tech_support_fix_valuation_v2({}) VALUES ({});".format(
            ','.join(params.keys()),
            ",".join(['%(' + key + ')s' for key in params.keys()])
        )
...
def get_summary():
    ...
    env.cr.execute(
        ...
        ORDER BY abs(initial_value - total_value) DESC""".format(
            "in %s" if specific_products else "is not null"
        ),
    ...
    for res in env.cr.dictfetchall():
        ...
        uom = product.uom_id
        summary.append(_SUMMARY_MESSAGE.format(
            product=product,
            cid=company.id,
...
def get_progress_report():
    ...
    if table_exists("tech_support_fix_valuation_v2"):
        ...
        env.cr.execute("""
            ...
            FROM tech_support_fix_valuation_v2
        """ + where + "GROUP BY company_id;", params)
