/*
 Stock valuation from quants at last purchase price and standard_price.
...
copy (
    ...
    select product_product.id as product_id,
        ...
        coalesce(ir_property.value_float, 0) as pmp,
        coalesce(ir_property.value_float, 0) * stock.qty as value_pmp,
        case
            WHEN (
            ...
            ELSE (
                coalesce(ir_property.value_float, 0) - coalesce(dpa.cost, 0)
            ) / coalesce(dpa.cost, 0)
        END as pmp_vs_dpa
    from stock
        ...
        left join ir_property on (
            ir_property.res_id = concat('product.product,', product_product.id)
            and ir_property.name = 'standard_price'
        )
        left join lateral (
            select distinct on (purchase_order_line.product_id) round(
                    purchase_order_line.price_subtotal / purchase_order_line.product_qty,
                    2
                ) as cost
            from purchase_order_line
                left join purchase_order on purchase_order_line.order_id = purchase_order.id
            where purchase_order_line.product_id = stock.product_id
                and purchase_order.state in ('purchase', 'done')
                and purchase_order_line.product_qty != 0
            order by purchase_order_line.product_id,
                date_order desc
        ) as dpa on true
