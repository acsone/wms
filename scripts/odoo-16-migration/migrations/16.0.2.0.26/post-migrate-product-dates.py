...
def _initialize_dates(env):
    query = """
        UPDATE product_template pt
            ...
            WHERE pt.expiration_time IS NOT NULL AND pt.expiration_time <> 0
    """
    ...
    query = """
        UPDATE product_template pt
            ...
            JOIN product_category pc on pc.id = pt.categ_id
            WHERE pt.use_time IS NOT NULL AND pt.use_time <> 0
    """
    openupgrade.logged_query(env.cr, query)
    ...
    query = """
        UPDATE product_template pt
            ...
            JOIN product_category pc on pc.id = pt.categ_id
            WHERE pt.removal_time IS NOT NULL AND pt.removal_time <> 0
    """
    openupgrade.logged_query(env.cr, query)
    ...
    query = """
        UPDATE product_template pt
            ...
            JOIN product_category pc on pc.id = pt.categ_id
            WHERE pt.alert_time IS NOT NULL AND pt.alert_time <> 0
    """
    openupgrade.logged_query(env.cr, query)
