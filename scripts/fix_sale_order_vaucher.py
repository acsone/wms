...
def write_log(so_id, so_name, so_line_id, so_line_name, state, refund_id,
    ...
    file_wr.write('%s;%s;%s;%s;%s;%s;%s\n' % (
        str(so_id), so_name, str(so_line_id), so_line_name, str(state),
        str(refund_id), reason))
    file_wr.close()
...
def create_new_refund(client, partner_id, refund_account_id, order_name):
    ...
    in_id = invoice_obj.create(
        {'reference_type': "none",
         ...
         'type': 'out_refund',
         'name': '%s-%s' % (order_name, 'Correction Cde Magento du 30/11/2018')
         })
    return in_id
...
def get_sale_order_to_correct(config=False):
    ...
    all_sale_order = sale_order_obj.search([('esb_ref', 'in', esb_ref),
                                            ('sale_channel', '=', 'web'),
                                            ('id', 'not in', old_id)])
