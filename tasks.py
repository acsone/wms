...
DOCKER_IMAGE = 'camptocamp/alcyon_odoo'
...
def bump(ctx, feature=False, patch=False):
    ...
    print('Version changed to {}, now:'.format(version))
    print('')
    print('* Clean {} (remove empty sections, '
          'whitespaces, ...)'.format(os.path.basename(HISTORY_FILE)))
    print('* Commit and tag with an annotated tag (git tag -a)')
