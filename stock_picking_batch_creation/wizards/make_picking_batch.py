...
class MakePickingBatch(models.TransientModel):
    ...
    def _get_picking_order_by(self):
        ...
        if self.group_pickings_by_partner:
            return "priority desc, scheduled_date asc, partner_id desc, id asc"
    ...
    def _get_picking_domain_for_device(self, device):
        return [
            ...
            ("nbr_picking_lines", "<=", self.maximum_number_of_preparation_lines),
        ]
    ...
    def _get_picking_domain_for_additional(self):
        ...
        user = self._first_picking.user_id
        if user:
        ...
        if self.group_pickings_by_partner:
            ...
            volume_domains.append(
                [
                    ("partner_id", "=", previous_picking.partner_id.id),
                    ("scheduled_date", "=", previous_picking.scheduled_date),
                    (
                        "volume",
                        "<=",
                        self._get_remaining_volume(previous_picking.partner_id),
                    ),
                ]
            )
    ...
    def _get_first_picking(self, user=None):
        domain = self._get_picking_domain_common(user=user)
        device_domains = []
    ...
    def _get_additional_picking(self):
        ...
        domain = AND(
            [domain, self._get_picking_domain_common(user=self._first_picking.user_id)]
        )
        return self._execute_search_pickings(domain, limit=1)
    ...
    def _raise_create_batch_not_possible(self):
        ...
        domain = OR(
            [
                self._get_picking_domain_common(),
                self._get_picking_domain_common(self.user_id),
            ]
        )
        limit = 1
    ...
    def _create_batch(self, raise_if_not_possible=False):
        ...
        first_picking = self._get_first_picking(user=self.user_id)
        if not first_picking:
            # if there is no picking for the user, we get the first picking
            # without user
    ...
    def _create_batch_values(self):
        ...
        return {
            ...
            "user_id": self._first_picking.user_id.id,
            "state": "draft",
