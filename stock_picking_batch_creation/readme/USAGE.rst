...
1. We search for a picking ready to be processed and that fit to the
...
3. While the defined limits are not reached, we look for one picking at a
   time that is ready to be processed and that fit the new criteria. The
...
lines remaining in the batch. While there is still some bins available and
in the batch and the search returns result, we look for a picking that fit
