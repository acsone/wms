...
long as the build is green!) and push it on the registry as `camptocamp/alcyon_odoo:x.y.z`
...
* Paul tests the image `camptocamp/alcyon_odoo:9.4.0` and oops, it seems he
...
2. Create local patch branch
    ...
    * Create PR to patch remote project branch.
        ```git
        camptocamp/alcyon/patch-x.y.next_z <- <user_name>/alcyon/<your_patch_branch>
        ```
