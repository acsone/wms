...
long as the build is green!) and push it on the registry as `camptocamp/alcyon_odoo:R.r.x.y.z`
...
developers, the current version is `10.0.1.3.2`:
...
* Paul merges the pull requests, prepares a new release and when he's done, he
    tags `master` with `10.0.1.4.0`
* Paul tests the image `camptocamp/alcyon_odoo:10.0.1.4.0` and oops, it seems he
...
* Liza works on another shiny feature:
    ...
    ```
* And Greg is assigned to fix a bug on the production server (now in `10.0.1.4.1`),
    so he will do 2 things:
    * create a patch branch **from** the production version:
        ```
        git checkout 10.0.1.4.1 -b patch-claim-typo
        git push greg/patch-claim-typo
        ```
    * ask Paul to create a new patch branch `patch-10.0.1.4.2`, on which he will
        propose his pull request
* Paul prepare a new release on the `patch-10.0.1.4.2` branch. Once released, Paul merges `patch-10.0.1.4.2` in `master`.
* At the end of the sprint, Paul prepares the next release `10.0.1.5.0` with the new Liza's feature and so on.

### Detailed instruction

1. Create remote patch branch

    * Figure out target version `number` in format `R.r.x.y.z` (`10.0.1.4.0`) for which
    ...
    * Create patch branch from target with name `patch-R.r.x.y.next_z`
        (`patch-10.0.1.4.1`). Where `next_z = z + 1` (`1 = 0 + 1`).
        ```git
        git checkout R.r.x.y.z -b patch-R.r.x.y.next_z
        ```
        example
        ```git
        git checkout 10.0.1.4.0 -b patch-10.0.1.4.1
        ```
    * Push new empty branch to repo.
        ...
        example
        ```git
        git push origin patch-10.0.1.4.1
        ```
...
2. Create local patch branch
    ...
    * Create PR to patch remote project branch.
        ```git
        camptocamp/alcyon/patch-R.r.x.y.next_z <- <user_name>/alcyon/<your_patch_branch>
        ```
