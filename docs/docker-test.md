...
1. Clone the project
        ...
        git clone git@github.com:camptocamp/alcyon_odoo.git alcyon
...
3. Start the composition
        ...
        cd alcyon
        docker-compose -f testers.latest.yml pull
        docker-compose -f testers.latest.yml up
...
4. In `testers.latest.yml` you might want to adapt the odoo `image` version (so replace `latest` by a specific tag or branch).

5. If you want to drop your database, run:

        docker-compose -f testers.latest.yml rm -v

If you want to start all the services (Odoo, Akeneo, WSO2), have a look on the [Full composition page](./full-composition.md).
