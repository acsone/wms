...
class StockQuant(models.Model):
    ...
    alert_date = fields.Datetime(
        related='lot_id.alert_date', store=True, readonly=True
    )

    use_date = fields.Datetime(
        related='lot_id.use_date', store=True, readonly=True
    )

    life_date = fields.Datetime(
        related='lot_id.life_date', store=True, readonly=True
    )
    ...
    def alert_quant_expired(self):
        ...
        if len(quants) > 0:
            ...
            template.with_context(quants_on_alert=quants).send_mail(
                quants[0].id
            )
