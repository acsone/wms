CREATE OR REPLACE VIEW zelapro_export_cadencier AS
  SELECT
    ...
    product.default_code AS SFDART,
    ...
    to_char(NOW() + format('%s days', COALESCE(info.delay, 0))::INTERVAL, 'DD/MM/YYYY') AS LIBDLI,
    '' AS SFDDMO,
    '' AS LIBDMO,
    CASE
  ...
  FROM product_supplierinfo AS info
    ...
    LEFT JOIN product_product AS product ON product.id = (SELECT min(id) FROM product_product WHERE product_tmpl_id = info.product_tmpl_id)
