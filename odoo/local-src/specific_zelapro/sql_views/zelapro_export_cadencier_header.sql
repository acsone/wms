CREATE OR REPLACE VIEW zelapro_export_cadencier_header AS
  SELECT
    ...
    to_char(NOW() + format('%s days', COALESCE(delivery_lead_time, 0))::INTERVAL, 'YYYYMMDD') AS SFEDLI,
    to_char(NOW() + format('%s days', COALESCE(delivery_lead_time, 0))::INTERVAL, 'DD/MM/YYYY') AS LIBDLI,
    '' AS SFEPDS,
    '' AS SFEMNT,
    '' AS SFEMNS,
