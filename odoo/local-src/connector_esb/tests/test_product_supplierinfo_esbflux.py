...
class ProductSupplierInfoEsbFluxTestCase(TransactionCase):
    ...
    def test_create_promotion_buyx_gety(self):
        ...
        r_flux = self.esbflux.search([('real_id', '=', r.id),
                                      ('flux', '=', 'buyxgety')])
        self.assertEqual(r.ratio_main_product, r_flux.ratio_main_product)
        self.assertEqual(r.ratio_promotional_product,
                         r_flux.ratio_promotional_product)
    ...
    def test_update_promotion_buyx_gety(self):
        ...
        r = self.esbflux.search([('real_id', '=', self.supinfo_1.id),
                                 ('flux', '=', 'buyxgety'),
                                 ('action', '=', 'delete')])
        # Check that the delete action entry is correct
        # It should contain the previous values
        self.assertEqual(r.ratio_main_product,
                         previous_ratio)
        self.assertEqual(r.ratio_promotional_product,
                         self.supinfo_1.ratio_promotional_product)
        self.assertEqual(r.product_tmpl_id.id,
                         self.supinfo_1.product_tmpl_id.id)
        self.assertEqual(r.date_start,
                         self.supinfo_1.date_start)
        self.assertEqual(r.date_end,
                         self.supinfo_1.date_end)
