...
class ProductSupplierInfoEsbFluxTestCase(TransactionCase):
    ...
    def test_reset_esbflux_table(self):
        ...
        r_flux = self.esbflux.search([('ratio_main_product', '=', 5),
                                      ('flux', '=', 'buyxgety')])
        self.assertEqual(
            self.supinfo_1.ratio_main_product,
            r_flux.ratio_main_product)
        self.assertEqual(
            self.supinfo_1.ratio_promotional_product,
            r_flux.ratio_promotional_product)
        self.assertEqual(
            self.supinfo_1.product_tmpl_id.id,
            r_flux.product_tmpl_id.id)
        ...
        # Check the buyxgety promo is correct
        r_flux = self.esbflux.search([('discount_sale', '=', 4),
        ...
        self.assertEqual(
            self.supinfo_3.product_tmpl_id.id,
            r_flux.product_tmpl_id.id)
