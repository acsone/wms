...
class ReportProductPriceImport(AbstractReportXlsx):
    def _get_ws_params(self, wb, data, products):
        ...
        template['purchase_price'] = {
            ...
            'data': {
                'type': 'number',
                'value': self._render("supplier.price"),
            },
            'width': 7,
        }
        template['sale_price'] = {
    ...
    def _product_prices_report(self, workbook, ws, ws_params, data, products):
        ...
        for product in products:
            supplier_info = default_supplier_infos.get(
                product, supplier_info_none
            )
...
ReportProductPriceImport(
    'report.product_price_import.xlsx', 'product.template'
)
