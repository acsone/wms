...
class ManageUOP(models.TransientModel):
    ...
    picking_uop = fields.Integer(
        'Picking UOP', related='picking_id.id', readonly=True
    )
    ...
    picking_zone_code = fields.Char(
        string='Picking Zone Code',
        related='picking_zone_id.code',
        readonly=True,
    )
    ...
    def to_to_validate_picking(self):
        ...
        line_to_update = self.picking_id.pack_operation_ids.filtered(
            ...
            and line.pack_lot_ids
        )

        self.is_change_lots = len(line_to_update)
        for pack_op in line_to_update:
            line_done = pack_op.pack_lot_ids.filtered(
                lambda pack_lot: pack_lot.qty
            )
            ...
            self.line_ids.create(
                {
                    ...
                    'lot_id': len(line_done) == 1
                    and line_done.lot_id.id
                    or None,
    ...
    def validate(self):
        ...
        if self.picking_zone_id == zone_medoc:
            try:
                self.picking_id.print_products_label(
                    printer_id=printer_toshiba.id
                )
                self.picking_id.print_packages_label(
...
class ViewPickingLine(models.TransientModel):
    ...
    location_checksum_right = fields.Char(
        'Location Checksum Right',
        related='location_id.bin_checksum_1',
        readonly=True,
    )
    location_checksum_left = fields.Char(
        'Location Checksum Left',
        related='location_id.bin_checksum_2',
        readonly=True,
    )
    lot_checksum = fields.Char(
        'Lot Checksum', related='lot_id.checksum', readonly=True
    )
    lot_voice_identifier = fields.Char(
        'Lot Voice Identifier',
        related='lot_id.voice_identifier',
        readonly=True,
    )
