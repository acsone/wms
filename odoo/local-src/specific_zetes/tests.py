...
def identification():
    """
    Send the command "REQU_USERCONTEXT" to retrieve the user name
    of the current pickeur.
    :return:
    """
...
def print_zones():
    """
    Send the command "REQU_REFDATA" to retrieve all zones.
    The method will parse the result and show only the number
...
def assignment():
    """
    1. Send the command "REQU_ASSIGNMENT" to retrieve the right picking
    Print the picking ID
    2. Assign the picking to the current user
    :return:
    """
    print '==== Assignment ===='
...
def get_itempicks(picking_id):
    """
    1. Send the request "REQU_ITEMPICK" to retrieve
    all picking lines for a picking.
    2. For each lines, set the picked quantity (with RESU_CATCHWEIGHT)
    and close the picking line (with RESU_ITEMPICK)
    :param picking_id: The picking ID
    :return:
    """
    print '==== Get itempicks ===='
...
def validate_picking(picking_id):
    """
    Send the command "RESU_ASSIGNMENT" to close the picking
    :param picking_id:
    :return:
    """
...
def print_picking(picking_id):
    """
    Send the request "REQU_PRINT" to print the picking like a passport (code 3)
    :param picking_id: The picking ID
    :return:
    """
...
def reset_picking_action(picking_id, move_id):
    """
    Send the request "RESU_ITEMPICK" to reset the quantity on the stock move
    :param picking_id: The picking ID
    :param move_id: The stock move ID
    :return:
    """
