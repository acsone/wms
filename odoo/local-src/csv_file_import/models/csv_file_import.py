    ...
    def _compute_last_action(self):
        for csv_file in self:
            if csv_file.logger_ids:
                ...
                csv_file.last_logger_datetime = \
                    csv_file.logger_ids[0].date_start
    ...
    def execute_imports(self):
        ...
        for csv_import in self:
            ...
            try:
                ...
                is_directory = self.check_directory(import_path,
                ...
                is_directory = self.check_directory(import_out_path,
                                                    create_if_missing=True)
                if not is_directory:
                    raise UserError(
                        _('The folder %s doesn\'t exist on the source')
                ...
                is_directory = self.check_directory(import_failure_path,
                                                    create_if_missing=True)
                if not is_directory:
                    raise UserError(
                        _('The folder %s doesn\'t exist on the source')
                ...
                # Retrieve files
                filename_pattern = \
                    self.get_filename_pattern(csv_import.filename)
                files_to_import = []
                files_imported = []
                files_not_imported = []
                ...
                if not files_imported and not files_not_imported:
                    ...
                    if files_not_imported:
                        ...
                        for file_not_imported, import_message in \
                                files_not_imported:
                            message += '- %s: %s\n' % \
