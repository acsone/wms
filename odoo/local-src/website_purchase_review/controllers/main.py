...
FILTERS = ['products_to_order', 'products_without_promo',
           'products_with_promo', 'product_name', 'next_product_id']
    ...
    def purchase_review(self, po, product=None, **kw):
        ...
        if po.date_planned_overwrite:
            date_planned_overwrite = \
                fields.Datetime.from_string(po.date_planned_overwrite)
            date_planned_overwrite_str = \
                fields.Date.to_string(date_planned_overwrite)
            render_values['date_planned_overwrite'] = \
                date_planned_overwrite_str

        if po.discount_global_overwrite:
            render_values['discount_global_overwrite'] = \
        ...
        if po_line:
            ...
            if render_values.get('date_planned_overwrite'):
                ...
                date_planned = \
                    datetime.now() + relativedelta(days=delivery_lead_time)
            else:
        ...
        return request.render("website_purchase_review.main_page",
                              render_values)
