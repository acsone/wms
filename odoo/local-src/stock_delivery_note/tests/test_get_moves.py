...
class TestStockDeliveryNoteGetMoves(SavepointCase):
    ...
    def test_group_shipping(self):
        ...
        self.assertEqual(
            order1.order_line[0].product_id, so1_moves[0].product_id
        )
        self.assertEqual(
            order2.order_line[0].product_id, so2_moves[0].product_id
        )
    ...
    def test_group_shipping_with_bo(self):
        ...
        self.assertEqual(
            order2.order_line[0].product_id, so2_bo_moves[0].product_id
        )
