...
class SalesService(Component):
    ...
    Provides methods to create and manage sale orders for Chronovet.

    date and confirmation date info are datetime formatted into ISO-8601
    with TZ info
    """

    _inherit = "base.chronovet.rest.service"
    _name = "sales.service"
    _usage = "sales"
    ...
    def create(self, **params):
        ...
        so = (
            self.env["sale.order"]
            .suspend_security()
            ._create_from_chonovet(params, self.chronovet_backend)
        )
        return {
            "id": so.chronovet_ref,
            "ref": so.name,
    ...
    def get(self, _id):
        ...
        res = (
            self.env["sale.order"]
            .suspend_security()
            .search_read(domain=[("chronovet_ref", "=", _id)], fields=self._read_fields)
        )
        if not res:
    ...
    def search(self, **params):
        ...
        ids = params.get("ids")
        if ids:
            domain.append(("chronovet_ref", "in", ids))
    ...
    def _read_fields(self):
        return ["chronovet_ref", "state", "confirmation_date", "name"]
