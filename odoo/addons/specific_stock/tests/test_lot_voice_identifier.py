...
class TestLotVoiceIdentifier(TransactionCase):
    ...
    @post_install(True)
    @at_install(False)
    def test_compute_voice_identifier(self):
        """
        This test will change the voice identifier size
        to have maximum 26 voice identifiers.
        After that we will generate 26 lots.
        Each lot should have a different voice identifier.
        After that we will create a new lot
        and voice identifier if the method raise an error.
        :return:
        """
        # Change the voice identifiers size
        self.env["ir.config_parameter"].set_param("lot_voice_identifier_size", 1)

        # Archive all lots
        self.env["stock.production.lot"].search([]).write({"is_archived": True})
        ...
        # All voice identifiers are assigned for this range (APA=>APD)
