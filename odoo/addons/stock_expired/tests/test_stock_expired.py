...
class TestStockExpired(TransactionCase):
    ...
    def test_4_process_quant_expired(self):
        ...
        for ignore in [True, False]:
            ...
            self.assertEqual(
                len(quants_expired), 0 if ignore else 2  # Quants expiration is ignored
            )
        ...
        for quant_expired in quants_expired:
            self.assertEqual(quant_expired.location_id.id, scrapped_location_id)
    ...
    def test_5_alert_quant_expired(self):
        ...
        mail_messages = self.mail_message_model.search([("model", "=", "stock.quant")])
        ...
        for ignore in [True, False]:
            ...
            self.assertEqual(
                len(mail_messages), 0 if ignore else 1  # Quants expiration is ignored
            )
            if not ignore:
