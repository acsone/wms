...
def _load_species_translations(cr):
    env = odoo.api.Environment(cr, odoo.SUPERUSER_ID, {})
    path = os.path.join(os.path.dirname(__file__), "static", "initial_translations.csv")
    with open(path) as csv_file:
        csv_reader = csv.DictReader(csv_file, delimiter=",")
        all_species_records = env["animal.species"].search([])
        species_by_en_name = {r.name: r for r in all_species_records}
        for r in csv_reader:
            record = species_by_en_name.get(r["en_GB"])
            if not record:
                continue
