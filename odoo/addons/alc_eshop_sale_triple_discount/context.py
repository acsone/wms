...
# to collect infomation from called by caller
...
class Context(object):
    ...
    def run(self, callable, *args, **kwargs):  # pylint: disable=redefined-builtin
    ...
    def copy(self):  # pylint: disable=method-required-super
        new = Context()
    ...
    def set(self, key, value):
        if not isinstance(key, ContextVar):
            raise TypeError("a ContextVar key was expected, got {!r}".format(key))
...
_updated_cart_line_ids_ctx = ContextVar("updated_cart_line_ids", None)
