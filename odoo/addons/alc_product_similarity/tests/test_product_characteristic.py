...
class TestProductCharacteristic(BaseCommon):
    ...
    def test_get_vector_indices_on_empty_characteristics(self):
        self.assertEqual(
            {},
            self.product_characteric.get_vector_indices(
                self.product_material.species_ids
            ),
        )

    def test_vector_indices_remain_constant(self):
        first_indices = self.product_characteric.get_vector_indices(
            self.product_vitamines.species_ids
        )
        second_indices = self.product_characteric.get_vector_indices(
            self.product_vitamines.species_ids
        )
    ...
    def test_vector_indices_fit_holes(self):
        ...
        new_product_vector_indices = self.product_characteric.get_vector_indices(
            self.product_vitamines.species_ids
        )

        self.assertEqual(
            set(new_product_vector_indices.values()) & to_delete_indices,
            to_delete_indices,
            f"Empty indices left over after deletion. New indices: {set(new_product_vector_indices.values())} | deleted indices: {to_delete_indices}",
        )
