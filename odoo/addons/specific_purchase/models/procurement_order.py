...
class ProcurementOrder(models.Model):
    ...
    def _get_orderpoint_domain(self, company_id=False):
        ...
        if self._context.get("type") == "by_suppliers":
            domain = [
                ("product_id.supplier_id.id", "in", self._context["supplier_ids"])
            ]
            ...
            if days_selected:
                ...
                while days_selected:
                    ...
                    domain.insert(0, "|")
                    domain.append(("product_id.supplier_id.%s" % day, "=", True))
        else:
    ...
    def _ensure_product_orderpoints(self, warehouse, products=None):
        Product = self.env["product.product"].with_context(warehouse=warehouse.id)
