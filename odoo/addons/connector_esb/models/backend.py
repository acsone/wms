...
class ESBBackend(models.Model):
    ...
    ws_url = fields.Char(
        string='Web Service Base URL', compute='_compute_from_env'
    )
    ws_user = fields.Char(
        string='Web Service User', compute='_compute_from_env'
    )
    ws_pwd = fields.Char(
        string='Web Service password', compute='_compute_from_env'
    )
    ...
    def cron_export_promotion_alcyon(self):
        ...
        backend._get_timestamp(
            'product.pricelist.item', 'promotion.alcyon'
        ).export()
    ...
    def cron_export_buy_x_get_y(self):
        backend = self.get_singleton()
        backend._get_timestamp(
            'product.supplierinfo.esbflux', 'buyx.gety'
        ).export()
