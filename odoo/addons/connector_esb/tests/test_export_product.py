...
class ExportProductTestCase(ESBXMLTestCase):
        ...
        self.all_records |= self.model.create(
            {
                ...
                "seller_ids": [
                    (0, 0, {"name": supplier.id, "product_code": "supplier001"})
                ],
        ...
        self.all_records |= self.model.create(
            {
                ...
                "taxes_id": [(4, tax2.id)],
                "seller_ids": [
                    (0, 0, {"name": supplier2.id, "product_code": "supplier002"})
                ],
        ...
        self.all_records |= self.model.create(
            {
                ...
                "taxes_id": [(4, tax.id)],
                "seller_ids": [
                    (0, 0, {"name": supplier.id, "product_code": "supplier003"})
                ],
                "route_ids": [
    ...
    def test_mapper_specific_fields(self):
        ...
        """
        self.p_cat.with_context({"lang": "nl_BE"}).warning_info = "Aandacht__" * 26
        self.p_cat.with_context({"lang": "fr_BE"}).warning_info = "Attention_" * 26
        self.p_cat.with_context({"lang": "de_DE"}).warning_info = "Aufmerksam" * 26
        rec = self.all_records[0]
        with self.backend.work_on(self.model._name, timestamp=self.timestamp) as work:
            mapper = work.component(usage="export.mapper")
