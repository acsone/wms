...
class ExportPromotionAlcyonTestCase(ESBXMLTestCase):
    def setUp(self):
        ...
        self.setup_records()
        self.maxDiff = None
        self.timestamp = self.env.ref("connector_esb.esb_timestamp_promotion_alcyon")
    ...
    def test_getitems(self):
        ...
        with self.backend.work_on(self.model._name, timestamp=self.timestamp) as work:
            ...
            self.assertEqual(len(items), len(self.discount_pricelist_1.item_ids) - 1)
    ...
    def test_export(self):
        ...
        with self.backend.work_on(self.model._name, timestamp=self.timestamp) as work:
            ...
            self.assertXmlEquivalentData(
                result, self.read_test_file("promotion_alcyon_1.xml"), "ProductType"
            )
