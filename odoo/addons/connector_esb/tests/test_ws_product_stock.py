...
class WSProductStockTestCase(ESBXMLTestCase):
    ...
    def setup_records(self):
        active = self.env.ref("alc_product_state.product_state_a")
        active.esb_ref = "A"
        discontinued = self.env.ref("alc_product_state.product_state_d")
        discontinued.esb_ref = "D"
        self.product1 = self.model.create(
            {"name": "Product1", "default_code": "exportable001", "state_id": active.id}
        )
        ...
        self.product3 = self.model.create(
            {"name": "Product3", "default_code": "exportable003", "state_id": active.id}
        )
