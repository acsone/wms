...
class WSStatisticsFormTestCase(ESBXMLTestCase):
    ...
    def setup_records(self):
        self.partner = self.env["res.partner"].create({"name": "Foo", "ref": "123"})
    ...
    def test_message(self):
        self.create_sale("2017-07-26", [(self.product1, 5, 1, 143.2, self.tax_20)])
        self.create_sale("2017-07-26", [(self.product2, 130, 123, 610.9, self.tax_20)])
    ...
    def test_filter_dates(self):
        self.create_sale("2017-07-20", [(self.product1, 5, 1, 143.2, self.tax_20)])
        self.create_sale("2017-07-26", [(self.product2, 130, 10, 6.0, self.tax_20)])
        self.create_sale("2017-07-30", [(self.product3, 1, 1, 10.0, self.tax_20)])

        backend = self.env["esb.backend"].get_singleton()
        with backend.work_on("res.partner") as work:
            component = work.component("ws.message.statistics.form")
            options = component.options_for_form(
                customer_ref="123", start=date(2017, 7, 22), end=date(2017, 7, 28)
            )
    ...
    def test_filter_product_type(self):
        ...
        with backend.work_on("res.partner") as work:
            component = work.component("ws.message.statistics.form")
            options = component.options_for_form(customer_ref="123", product_type="ALI")
            data = component._data_for_message(options)
    ...
    def test_filter_supplier(self):
        ...
        self.create_sale("2017-07-20", [(self.product4, 5, 1, 143.2, self.tax_20)])
        self.create_sale("2017-07-26", [(self.product2, 130, 10, 6.0, self.tax_20)])
        self.create_sale("2017-07-30", [(self.product3, 1, 1, 10.0, self.tax_20)])
    ...
    def test_language(self):
        ...
        with backend.work_on("res.partner") as work:
            component = work.component("ws.message.statistics.form")
            options = component.options_for_form(customer_ref="123", language="TLH")
            data = component._data_for_message(options)
