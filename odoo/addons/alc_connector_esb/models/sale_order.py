...
class SaleOrder(sale_order.SaleOrder):
    ...
    def _ws_create_new(self, data, creation_date):
        ...
        for line in self._ws_create_order_line_data(data)[:]:
            if line["product_id"] == "":
                ...
                order.message_post(
                    body=_(
                        "Product with %(code_type)s : %(code_searched)s was not found",
                        code_type=line["code_type"],
                        code_searched=line["code_searched"],
                    )
                )
                continue
    ...
    def _ws_get_partner(self, ref):
        ...
        if not partner:
            raise exceptions.MissingError(
                _("No match found for customer_id: %(ref)s", ref=ref)
            )
        return partner
    ...
    def _ws_get_date_order(ws_date):
        ...
        now_date, _spc, now_time = str(fields.Datetime.now()).partition(" ")
        if ws_date == now_date:
    ...
    def _ws_create_order_data(self, data):
        ...
        if partner_ref in self.env["res.partner"].newpharma_refs:
            ...
            order_data["sale_channel_id"] = self.env.ref(
                "alc_connector_esb.sale_channel_newpharma"
            ).id
        else:
    ...
    def _ws_create_order_line_data(self, data):
        ...
        for line in data["lines"]:
            ...
            if len(product) > 1:
                ...
                message = (
                    "Webservice new saleorder, several"
                    " products with the same sku/cnk %(product_code)s found"
                )
                raise exceptions.UserError(_(message, product_code=product_code))
            sol = {}
            if len(product) == 1:
