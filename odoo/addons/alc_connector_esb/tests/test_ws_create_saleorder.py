...
class TestSaleController(TransactionCase):
    ...
    def setup_records(cls):
        ...
        # cls.partner.property_product_pricelist = cls.pricelist_1.id
        # cls.partner_newpharma.property_product_pricelist = cls.pricelist_1.id
    ...
    def test_create_saleorder(self):
        ...
        expected = {
            ...
            "team_id": web_team,
            "sale_channel_id": self.env.ref("alc_sale_channel.sale_channel_web"),
            "suite_name": "iamsuitename",
        }
        for k, v in expected.items():
            if isinstance(v, float):
                self.assertAlmostEqual(order[k], v, msg=k)
            else:
                self.assertEqual(order[k], v)
        # free line: to be skipped
        self.assertEqual(len(order.order_line), 1)
        # Confirmtation/order date are the time of creation in Odoo by the ws
        self.assertTrue(starting_date <= order.date_order <= fields.Datetime.now())
    ...
    def test_create_saleorder_no_notify(self):
        ...
        self.env["sale.order"]._patch_method("message_post_with_view", mock.MagicMock())
    ...
    @unittest.skip("Condifmation is synchronous")
    ...
    def test_create_saleorder_newpharama(self):
        ...
        self.assertEqual(order.sale_channel_id.code, "newpharma")
