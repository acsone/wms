...
class TestStockPickingValidation(GLSCommonFeatures):
    @classmethod
    def setUpClass(cls):
        super(TestStockPickingValidation, cls).setUpClass()
    ...
    def _create_pick_ship(cls):
        ...
        for pack in pick_medoc.pack_operation_ids:
        ...
        for pack in pick_alim.pack_operation_ids:
            pack.qty_done = pack.product_qty
    ...
    def test_00(self):
        ...
        Expected Result: No way to validate the ship by standard mecanism.
    ...
    def test_02(self):
        """
        Data:TwoO, picks are done. Ship has to be done
        Test Case: We process one medicine pack outin the first time
                    Ship cannot be validated. We then process food. Ship 1 can be validated, ship 2 can't
                    We process the second medicine pack one to allow second shipping validation
        ...
        pick_medoc1.force_assign()
        for pack in pick_medoc1.pack_operation_ids:
            pack.qty_done = pack.product_qty

        pick_medoc1.put_in_pack()
        pick_medoc1.do_transfer()

        ship1 = sale1.mapped("picking_ids").filtered(
            lambda p: p.picking_type_code == "outgoing"
        )

        pack0 = ship.pack_operation_pack_ids[0]
        pack0.qty_done = pack0.product_qty
    ...
    def test_03(self):
        ...
        """
        delivery_template2 = self.env["round.template"].create(
            {"name": "Unittest delivery template2"}
        )
        carrier2 = self.env["delivery.carrier"].create(
            {
                ...
                "delivery_type": "fixed",
                "fixed_price": 10.0,
                "delivery_template_id": delivery_template2.id,
            }
        )

        sale = self._confirm_sale_order(
            partner=self.partner1, product=[self.p1, self.p3], carrier_id=carrier2.id,
        )
