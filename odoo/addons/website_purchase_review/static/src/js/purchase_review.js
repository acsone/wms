...
odoo.define("website_purchase_review.main_page", function(require) {
  ...
  function init_listeners() {
    ...
    $("#product_qty").on("input", function() {
      var average_three_months_consumption = parseFloat(
        $("#average_three_months_consumption")
          .text()
          .replace(/\s+/g, "")
      );
      if (average_three_months_consumption == 0) {
        ...
        var virtual_available = parseFloat(
          $("#virtual_available")
            .text()
            .replace(/\s+/g, "")
        );
  ...
  function _set_loaded_products(products) {
    $.each(products, function(index) {
      products_list.push({
        ...
        picking_zone: products[index].picking_zone,
      });
    });
  ...
  function refresh_list() {
    ...
    var picking_zones_checked = [];
    $("input[id*=picking_zone_]").each(function() {
      if ($(this).is(":checked")) {
        picking_zones_checked.push(this.id);
      }
    });
    current_products_list = products_list.filter(function(product) {
      if (
        picking_zones_checked.length &&
        !picking_zones_checked.includes(product.picking_zone)
      ) {
        return false;
      }
