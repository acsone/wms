...
class CreditCommunication(models.TransientModel):
    ...
    def _generate_comm_from_credit_lines(self, lines):
        comms = super(CreditCommunication, self)._generate_comm_from_credit_lines(lines)
        comms._get_payment()
        return comms
                ...
                .search(
                    [
                        ...
                        (
                            "id",
                            "not in",
                            self.mapped("credit_control_line_ids.move_line_id").ids,
                        ),
                        (
                            "company_id",
                            "in",
                            self.mapped("credit_control_line_ids.company_id").ids,
                        ),
                    ]
                )
    ...
    def get_consolidate_lines(self):
        ...
        return sorted(lines, key=lambda r: fields.Date.from_string(r["date_entry"]))
