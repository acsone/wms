...
class StockInventory(models.Model):
    ...
    def prepare_inventory(self):
        for inventory in self:
            ...
            lines_values = [(0, 0, line_values) for line_values in inventory_lines]
    ...
    def _get_inventory_lines_with_lots_values(self):
        ...
        query = """
          ...
          GROUP BY
           ...
           package_id,
           partner_id"""
        self.env.cr.execute(query, (tuple(self.product_ids.ids), tuple(locations.ids)))

        for product_data in self.env.cr.dictfetchall():
            # replace the None the dictionary by False,
            # because falsy values are tested later on
            void_fields = [item[0] for item in product_data.items() if item[1] is None]
        ...
        exhausted_vals = self._get_empty_product_bin(self.product_ids, quant_products)
