...
class ResPartner(models.Model):
    ...
    date_start_contract_alcyonnaire = fields.Date(
        string="Date start contract", track_visibility="onchange"
    )
    ...
    def _check_date_contract(self):
        for partner in self:
            ...
            if (
                partner.date_start_contract_alcyonnaire
                and partner.date_start_contract_alcyonnaire > fields.Date.today()
            ):
                raise ValidationError(
            ...
            if (
                partner.date_end_contract_alcyonnaire
                and partner.date_end_contract_alcyonnaire > fields.Date.today()
            ):
        ...
        for partner in self:
            ...
            if partner.is_alcyonnaire:
                if partner.date_start_contract_alcyonnaire:
                    if partner.date_start_contract_alcyonnaire <= fields.Date.today() and (
                        not partner.date_end_contract_alcyonnaire
                        or partner.date_end_contract_alcyonnaire > fields.Date.today()
                    ):
    ...
    def _check_write_contract_date_allowed(self, vals):
        for partner in self:
            ...
            date_end_contract_alcyonnaire = vals.get(
                "date_end_contract_alcyonnaire", partner.date_end_contract_alcyonnaire,
            )
    ...
    def create(self, vals):
        ...
        res._check_write_contract_date_allowed(vals)
        return res
