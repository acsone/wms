...
class ClusterPickingStockIssue(ClusterPickingCommonCase):
    ...
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingStockIssue, cls).setUpClassBaseData(*args, **kwargs)
        ...
        cls.moves = cls.batch.mapped("picking_ids.move_lines").sorted("id")
    ...
    def _stock_issue(self, operation, next_operation_func=None, lot_id=None):
        batch = operation.picking_id.batch_id
        params = {"picking_batch_id": batch.id, "operation_id": operation.id}
        if lot_id:
            params["lot_id"] = lot_id
        response = self.service.dispatch("stock_issue", params=params)
        # use a function/lambda to delay the read of the next operation,
        # when calling _stock_issue(), the move_line may not exist and
        # be created during the call to the stock_issue service
        if next_operation_func:
            self.assert_response(
                response,
                next_state="start_operation",
                data=self._operation_data(next_operation_func()),
            )
        else:
            self.assert_response(
                response,
    ...
    def assert_location_qty_and_reserved(
        ...
        # takes only quants linked to batch
        location_quants = (
            self.env["stock.quant"]
            .search(quant_domain)
            .filtered("reservation_id.picking_id.batch_id")
        )
        self.assertEqual(sum(location_quants.mapped("qty")), expected_reserved_qty)
    ...
    def assert_stock_issue_reserved_qties(
        ...
        self.assertEqual(sum(quants.mapped("qty")), remaining_qty)
    ...
    def test_stock_issue_with_other_batch(self):
        ...
        self.assertEqual(
            set(self.batch_other.mapped("picking_ids.state")), {"assigned"}
        )
        ...
        self._stock_issue(self.move3.pack_operation_ids)
        ...
        expected_reserved_qty = (
            ...
            + sum(
                self.batch_other.picking_ids.pack_operation_ids.filtered(
    ...
    def test_stock_issue_reserve_elsewhere(self):
        ...
        # the operator picks the first operation
        self._set_dest_package_and_done(
            self.move1.pack_operation_ids, self.dest_package
        )

        # and has a stock issue on the second operation
        # because there is nothing inside, they declare a stock issue
        self._stock_issue(
            self.move2.pack_operation_ids,
            next_operation_func=lambda: self.move2.pack_operation_ids,
        )
    ...
    @unittest.skip("Partial package picking not supported in odoo 10")
    def test_stock_issue_similar_move_with_picked_line(self):
        """Stock issue on the remaining of a operation on partial move
        ...
        A new move operation of 1 is created to pick in the first package: we
        ...
        (self.move1 | self.move2 | self.move3).action_assign()
        self._simulate_batch_selected(self.batch, fill_stock=False)
        self.assertEqual(set(self.batch.picking_ids.mapped("state")), {"assigned"})

        pick_line1, pick_line2 = self.move1.pack_operation_ids
        ...
        # on the third move, the operator can't pick anymore in shelf1
        # because there is nothing inside, they declare a stock issue
        self._stock_issue(new_line, next_operation_func=lambda: pick_line2)
