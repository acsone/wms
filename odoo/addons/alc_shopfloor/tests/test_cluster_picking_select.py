...
class ClusterPickingSelectionCase(ClusterPickingCommonCase):
    ...
    * /cluster_picking/unassign
    ...
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingSelectionCase, cls).setUpClassBaseData(*args, **kwargs)
    ...
    def test_list_batch(self):
        ...
        self.assertEqual(
            self.env["stock.picking.wave"].search([]),
...
class ClusterPickingSelectedCase(ClusterPickingCommonCase):
    ...
    @classmethod
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingSelectedCase, cls).setUpClassBaseData(*args, **kwargs)
    ...
    def test_lines_order(self):
        ...
        # future = fields.Datetime.to_string(future)
        # Change dates
        move1 = picking1.move_lines[0]
        move1_operation = move1.pack_operation_ids[0]
        move1.write({"date_expected": today})
        (batch.picking_ids.mapped("move_lines") - move1).write(
            {"date_expected": future}
        )

        operations = self.service._operations_for_picking_batch(batch)
        order_mapping = {line: i for i, line in enumerate(operations)}

        # Today line comes first
        self.assertEqual(order_mapping[move1_operation], 0)
        # swap dates
        move1.write({"date_expected": future})
        (batch.picking_ids.mapped("move_lines") - move1).write({"date_expected": today})

        operations = self.service._operations_for_picking_batch(batch)
        order_mapping = {line: i for i, line in enumerate(operations)}
        self.assertEqual(order_mapping[move1_operation], len(operations) - 1)
    ...
    def test_confirm_start_ok(self):
        ...
        first_operation = picking.pack_operation_ids[0]
    ...
    def test_confirm_start_all_is_done(self):
        ...
        self._set_dest_package_and_done(
            self.batch.mapped("picking_ids.pack_operation_ids"),
