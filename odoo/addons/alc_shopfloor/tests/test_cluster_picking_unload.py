...
class ClusterPickingUnloadingCommonCase(ClusterPickingCommonCase):
    ...
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingUnloadingCommonCase, cls).setUpClassBaseData(
            *args, **kwargs
        )
        ...
        two_lines_product_a = cls.two_lines_picking.pack_operation_ids.filtered(
            lambda line: line.product_id == cls.product_a
        )
        two_lines_product_b = (
            cls.two_lines_picking.pack_operation_ids - two_lines_product_a
        )
        # force order of move lines to use in tests
        cls.pack_operation_ids = (
            cls.one_line_picking.pack_operation_ids
            + two_lines_product_a
            + two_lines_product_b
        )
...
class ClusterPickingPrepareUnloadCase(ClusterPickingUnloadingCommonCase):
    ...
    def test_prepare_unload_different_dest(self):
        ...
        self._set_dest_package_and_done(operations[:2], self.bin1)
        self._set_dest_package_and_done(operations[2:], self.bin2)
        operations[:2].write({"location_dest_id": self.packing_a_location.id})
        operations[2:].write({"location_dest_id": self.packing_b_location.id})
        response = self.service.dispatch(
            "prepare_unload", params={"picking_batch_id": self.batch.id}
        )
...
class ClusterPickingSetDestinationAllCase(ClusterPickingUnloadingCommonCase):
    ...
    def test_set_destination_all_remaining_lines(self):
        ...
        operations_to_unload = self.pack_operation_ids[:2]
        self._set_dest_package_and_done(operations_to_unload, self.bin1)
        operations_to_unload.write({"location_dest_id": self.packing_location.id})
        ...
        self.assert_response(
            ...
            data=self._operation_data(self.pack_operation_ids[2]),
    ...
    def test_set_destination_all_picking_unassigned(self):
        ...
        lines = self.one_line_picking.pack_operation_ids
    ...
    def test_set_destination_all_but_different_dest(self):
        ...
        operations = self.pack_operation_ids
        self._set_dest_package_and_done(operations, self.bin1)
        operations[:2].write({"location_dest_id": self.packing_a_location.id})
        operations[2:].write({"location_dest_id": self.packing_b_location.id})
    ...
    def test_set_destination_all_error_location_not_found(self):
        ...
        operations = self.pack_operation_ids
        self._set_dest_package_and_done(operations, self.bin1)
        operations.write({"location_dest_id": self.packing_a_location.id})
    ...
    def test_set_destination_all_error_location_move_invalid(self):
        ...
        operations = self.pack_operation_ids
        self._set_dest_package_and_done(operations, self.bin1)
        operations[0].move_ids.location_dest_id = self.packing_a_location
        operations[0].picking_id.location_dest_id = self.packing_a_location
    ...
    def test_set_destination_all_need_confirmation(self):
        ...
        operationss = self.pack_operation_ids
        self._set_dest_package_and_done(operationss, self.bin1)
        operationss.write({"location_dest_id": self.packing_a_location.id})
        ...
        location = operationss[0].location_dest_id
        data = self._data_for_batch(self.batch, location)
        self.assert_response(
...
class ClusterPickingUnloadScanPackCase(ClusterPickingUnloadingCommonCase):
    ...
    @classmethod
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingUnloadScanPackCase, cls).setUpClassBaseData(*args, **kwargs)
        cls._set_dest_package_and_done(cls.pack_operation_ids, cls.bin1)
        cls.pack_operation_ids[:2].write(
            {"location_dest_id": cls.packing_a_location.id}
        )
        cls.pack_operation_ids[2:].write(
            {"location_dest_id": cls.packing_b_location.id}
        )
    ...
    def test_unload_scan_pack_ok(self):
        ...
        location = self.pack_operation_ids[0].location_dest_id
        data = self._data_for_batch(self.batch, location, pack=self.bin1)
        self.assert_response(
...
class ClusterPickingUnloadScanDestinationCase(ClusterPickingUnloadingCommonCase):
    ...
    @classmethod
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingUnloadScanDestinationCase, cls).setUpClassBaseData(
            *args, **kwargs
        )
        cls.bin1_lines = cls.pack_operation_ids[:1]
        cls.bin2_lines = cls.pack_operation_ids[1:]
    ...
    def test_unload_scan_destination_with_confirmation(self):
        ...
        self.assertRecordValues(
            self.two_lines_picking.pack_operation_ids,
            [
                {"location_dest_id": self.packing_a_location.id},
                {"location_dest_id": self.packing_a_location.id},
            ],
    ...
    def test_unload_scan_destination_completion_info(self):
        ...
        dest_location = picking.pack_operation_ids.location_dest_id
