...
class ActionsDataCaseBase(CommonCase, ActionsDataTestMixin):
    ...
    def setUpClassVars(cls):
        super(ActionsDataCaseBase, cls).setUpClassVars()
        cls.wh = cls.env.ref("stock.warehouse0")
        cls.picking_type = cls.wh.out_type_id
        cls.picking_type.sudo().default_location_dest_id = cls.env.ref(
    ...
    def setUpClassBaseData(cls):
        super(ActionsDataCaseBase, cls).setUpClassBaseData()
        cls.packaging_type = (
        ...
        cls.picking.min_date = "2020-08-03"
...
class ActionsDataDetailCaseBase(ActionsDataCaseBase):
    @classmethod
    def setUpClassBaseData(cls):
        super(ActionsDataDetailCaseBase, cls).setUpClassBaseData()
        cls.lot = cls.env["stock.production.lot"].create(
            {"product_id": cls.product_b.id}
        )
        cls.package = cls.move_a.linked_move_operation_ids.operation_id.package_id

    @classmethod
    def setUpClassVars(cls):
        super(ActionsDataDetailCaseBase, cls).setUpClassVars()
        cls.storage_type_pallet = cls.env.ref(
            "stock_storage_type.package_storage_type_pallets"
        )

    def _expected_location_detail(self, record, **kw):
        v = dict(**self._expected_location(record))
        v.update(
            {
    ...
    def _expected_product_detail(self, record, **kw):
        ...
        if kw.get("full"):
            detail.update(
                {
                    "image": "/web/image/product.product/{}/image_small".format(
                        record.id
                    )
        ...
        v = dict(**self._expected_product(record))
        v.update(detail)
        return v
