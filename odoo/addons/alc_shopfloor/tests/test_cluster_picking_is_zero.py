...
class ClusterPickingIsZeroCase(ClusterPickingCommonCase):
    ...
    def setUpClassBaseData(cls, *args, **kwargs):
        super(ClusterPickingIsZeroCase, cls).setUpClassBaseData(*args, **kwargs)
        ...
        cls.operation = cls.picking.pack_operation_ids[0]
        cls.next_operation = cls.picking.pack_operation_ids[1]
        cls.bin1 = cls.env["stock.quant.package"].create({})
        cls._update_qty_in_location(
            cls.operation.location_id,
            cls.operation.product_id,
            cls.operation.product_qty,
        )
        # we already scan and put the first line in bin1, at this point the
        # system see the location is empty and reach "zero_check"
        cls._set_dest_package_and_done(cls.operation, cls.bin1)
    ...
    def test_is_zero_is_empty(self):
        ...
        self.assert_response(
            ...
            data=self._operation_data(self.next_operation),
            message={
                "message_type": "success",
                "body": "{} {} put in {}".format(
                    self.operation.qty_done,
                    self.operation.product_id.display_name,
                    self.bin1.name,
                ),
            },
        )
    ...
    def test_is_zero_is_not_empty(self):
        ...
        inventory = self.env["stock.inventory"].search(
            [
                ("location_id", "=", self.operation.location_id.id),
                ("product_id", "=", self.operation.product_id.id),
                ("state", "=", "draft"),
            ]
        )
