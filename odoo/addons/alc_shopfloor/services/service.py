...
class BaseShopfloorService(AbstractComponent):
    ...
    @property
    def search_pack_operation(self):
...
class BaseShopfloorProcess(AbstractComponent):
    ...
    def search_pack_operation(self):
        ...
        # every time.
        return self._actions_for(
            "search_pack_operation", picking_types=self.picking_types
        )
    ...
    def is_dest_location_valid(self, moves, location):
        ...
        return location.is_sublocation_of(
            moves.mapped("picking_id.location_dest_id"), func=all
        ) or location.is_sublocation_of(moves.mapped("location_dest_id"), func=all)
