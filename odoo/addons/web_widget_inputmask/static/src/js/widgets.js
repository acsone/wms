odoo.define("web.inputmask_widgets", function(require) {
  ...
  function mask_attrs(attrs) {
    ...
    attrsMask = Object.keys(attrs).reduce(function(filtered, key) {
      if (key.indexOf(keyMask) !== -1) filtered[key] = attrs[key];
      return filtered;
    }, {});
    if (attrsMask === null)
      console.warn(
        "The widget Mask expects the 'data-inputmask[-attribute]' attrsMask!"
      );
    return attrsMask;
  }
  ...
  var AbstractFieldMask = {
    ...
    render_value: function() {
      ...
      if (this.$input !== undefined) this.$input.inputmask(this.maskType);
      else this.$el.val(this.$el.text());
  ...
  var ColumnMask = list_widget_registry.get("field.char").extend({
    ...
    init: function(id, tag, attrs) {
      ...
      if (this.attrsMask)
        this.$mask = $(
          jQuery.parseHTML(QWeb.render("Widget.mask", {widget: this}))
        ).inputmask(undefined, {placeholder: "", greedy: false});
    },
  ...
  var MaskWidget = kanban_widgets.AbstractField.extend({
    ...
    renderElement: function() {
      ...
      if (this.$mask) this.$mask.val(value);
      value = this.$mask.val();
      this.$el.text(value);
    },
  });
  ...
  core.form_widget_registry
    ...
    .add("mask_regex", FieldRegexMask)
    .add("regex_mask", FieldRegexMask);
  // Mask_regex @ Deprecated latest version FOR name conversion!
