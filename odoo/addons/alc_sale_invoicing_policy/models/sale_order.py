...
class SaleOrder(models.Model):
    ...
    def _cron_invoice_makeall(self, day):
        date = datetime.today()
        if day == 0:
            # Cron every day, execute function only if the day is the one selected for the partner
            # and every 15 days
            invoice_frequencies = ["14_days"]
        elif day == -1:
        ...
        WHERE
            ...
            AND COALESCE(pm.invoice_frequency, partner.invoice_frequency) IN %(invoice_frequencies)s
            %(partner_invoice_date)s
        GROUP BY
            so.partner_invoice_id,
            so.payment_mode_id
            """
        query_params = {
            "invoice_frequencies": tuple(invoice_frequencies),
            "partner_invoice_date": AsIs("AND partner.next_invoice_date <= DATE(NOW())")
            if invoice_frequencies == ["14_days"]
            else AsIs(""),
        }
    ...
    def _job_invoices_by_partner(self, partner_id, payment_mode_id, date_invoice):
        ...
        with self._auto_join(["order_line"]):
            ...
            if partner.invoice_frequency == "14_days":
                next_invoice_date = datetime.strptime(
                    date_invoice, "%Y-%m-%d"
                ) + timedelta(days=14)
                next_invoice_date = next_invoice_date.replace(hour=22)
                partner.write({"next_invoice_date": next_invoice_date})
