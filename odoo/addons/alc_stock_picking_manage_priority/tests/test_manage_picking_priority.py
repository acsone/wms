...
class TestManageMovePriority(TransactionCase):
    ...
    def test_picking_priority(self):
        """
        - Create and confirm a picking then you can't change its priority.

        - Enable the move priority management but still can't change the picking's
          priority
        - Unlock the picking then you can change the its priority
        - Validate the move and you can't change the priority anymore
        """
        ...
        self.assertTrue(pick.is_priority_editable)  # priority editable
        ...
        self.assertFalse(pick.is_priority_editable)  # priority not editable
        # try to change the priority again
        with self.assertRaises(ValidationError, msg=msg):
            pick.priority = "1"
