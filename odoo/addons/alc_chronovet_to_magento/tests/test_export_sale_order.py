...
class TestExportSaleOrder(common.ESBTestCase):
    ...
    def test_00(self):
        ...
        with self.backend.work_on("sale.order") as work:
            ...
            values = mapper.map_record(self.chronovet_order).values()
        self.assertEqual(
            values["customer_id"],
    ...
    def test_01(self, post):
        ...
        self.chronovet_order.action_confirm()
        with self.backend.work_on("sale.order") as work:
            exporter = work.component(usage="record.exporter")
            exporter.run(self.chronovet_order)
        post.assert_called_once()
        self.assertEqual(self.chronovet_order.esb_ref, "1000000348")
