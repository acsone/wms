...
class TestDeliveryRoundAssign(TestDeliveryRoundAssignMixin):
    ...
    def test_deliveryround_carrier(self):
        ...
        for picking in sale.picking_ids:
            self.assertEqual(picking.delivery_round_id.id, self.delivery_round_2.id)
    ...
    def test_force_deliveryround_partially_available(self):
        ...
        for picking in sale.picking_ids:
            self.assertEqual(picking.delivery_round_id.id, self.delivery_round_1.id)
    ...
    def test_manual_change_delivery_round_fixed_to_std(self):
        ...
        ship = sale.picking_ids.filtered(lambda p: p.picking_type_id.code == "outgoing")
        ...
        for picking in sale.picking_ids:
            self.assertEqual(picking.delivery_round_id.id, self.delivery_round_1.id)
            self.assertEqual(picking.group_id.carrier_id, carrier_manual_change)
...
class TestRoundWithCustomerWorkingSchedule(
    TestCustomerWorkingScheduleBase, DeliveryRoundTestCase, TestDeliveryRoundAssignMixin
):
