...
class TestPickingTotal(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env.company.delivery_slip_packages_display_mode = "package"
    ...
    def test_delivery(self):
        ...
        result = ship_move.picking_id.parcels_and_items_per_category

        self.assertEqual(result.get("parcels"), {str(self.medocs.id): 1})
        self.assertDictEqual(
            result.get("items"), {str(self.aliments.id): 5, "false": 10.0}
        )
        self.assertEqual(result.get("parcels_total"), 1)
        self.assertEqual(result.get("items_total"), 15.0)
