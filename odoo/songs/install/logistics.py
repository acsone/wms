...
def company_settings(ctx):
    ...
    company.write(
        {
            'delivery_terms_conditions': "<p>Pour être livré le jour de livraison prévu, veuillez passer "
            "vos commandes :<br/>"
...
def settings_lot_base_date(ctx):
    ...
    # Default invoice
    define_settings(
        ctx, 'stock.config.settings', {'production_lot_base_date': 'life'}
    )
...
def create_locations(ctx):
    ...
    reserves = [
        (
            '__setup__.stock_location_reserve_ali',
            'Réserve Aliments',
    ...
    locations = [
        ('__setup__.stock_location_materiel', 'Matériel', False, loc_stock.id),
        (
            '__setup__.stock_location_ali',
            'Aliments',
            ctx.env.ref('__setup__.stock_location_reserve_ali').id,
            loc_stock.id,
        ),
        (
            '__setup__.stock_location_medoc',
            'Médicaments',
            ctx.env.ref('__setup__.stock_location_reserve_medoc').id,
            loc_stock.id,
        ),
        ('__setup__.stock_location_froid', 'Froid', False, loc_stock.id),
    ]
    ...
    locations = [
        (
            '__setup__.stock_location_frigo',
            'Frigo',
            False,
    ...
    parkings = [
        ...
        ('__setup__.stock_location_parking_ali', 'Parking Aliments'),
    ...
    onorders = [
        ...
        ('__setup__.stock_location_order_medoc', 'Achetés-Vendus Médicaments'),
    ...
    create_or_update(
        ctx,
        Location,
        '__setup__.stock_location_destroyed',
        {
            'name': 'Détruit',
            'location_id': loc_partner.id,
            'usage': 'customer',
    ...
    create_or_update(
        ctx,
        Location,
        '__setup__.mig_purchase_reception',
        {
...
def create_putaway(ctx):
    ...
    create_or_update(
        ...
        '__setup__.stock_putaway_input_mto',
        {
            ...
            'fixed_location_id': ref('__setup__.stock_location_onorder').id,
        },
    )
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_input_new',
        {
            ...
            'fixed_location_id': ref('__setup__.stock_location_new').id,
        },
    )
    create_or_update(
        ctx,
    ...
    create_or_update(
        ...
        {'name': 'Achetés-Vendus', 'method': 'fixed'},
    )
    # Fixed Locations Per Routes
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_onorder_ali',
        {
            ...
            'route_id': ref('__setup__.stock_location_route_pick_ali').id,
            'fixed_location_id': ref('__setup__.stock_location_order_ali').id,
        },
    )
    create_or_update(
        ctx,
    ...
    create_or_update(
        ...
        '__setup__.stock_putaway_onorder_mat',
        {
            'putaway_id': ref('__setup__.stock_putaway_onorder').id,
            'route_id': ref('__setup__.stock_location_route_pick_materiel').id,
            'fixed_location_id': ref('__setup__.stock_location_order_mat').id,
        },
    )
    create_or_update(
        ctx,
    ...
    create_or_update(
        ctx,
        'product.putaway',
        '__setup__.stock_putaway_new',
        {'name': 'Nouveautés', 'method': 'fixed'},
    )
    # Fixed Locations Per Routes
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_new_ali',
        {
            ...
            'route_id': ref('__setup__.stock_location_route_pick_ali').id,
            'fixed_location_id': ref('__setup__.stock_location_new_ali').id,
        },
    )
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_new_med',
        {
            'putaway_id': ref('__setup__.stock_putaway_new').id,
            'route_id': ref('__setup__.stock_location_route_pick_medoc').id,
            'fixed_location_id': ref('__setup__.stock_location_new_medoc').id,
        },
    )
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_new_mat',
        {
            'putaway_id': ref('__setup__.stock_putaway_new').id,
            'route_id': ref('__setup__.stock_location_route_pick_materiel').id,
            'fixed_location_id': ref('__setup__.stock_location_new_mat').id,
        },
    )
    create_or_update(
        ctx,
        ...
        '__setup__.stock_putaway_new_froid',
        {
            'putaway_id': ref('__setup__.stock_putaway_new').id,
            'route_id': ref('__setup__.stock_location_route_pick_froid').id,
            'fixed_location_id': ref('__setup__.stock_location_new_frigo').id,
        },
    )
    # Fixed Locations Per Categories
    news = [
...
def create_picking_types(ctx):
    ...
    location_parking_ali = ctx.env.ref('__setup__.stock_location_parking_ali')
    ...
    types = [
        ...
        },
        {
            ...
            'sequence': 31,
            'picking_zone_id': ctx.env.ref(
                '__setup__.picking_zone_aliments'
            ).id,
            'zetes_picking_type': RANGEMENT_ASSIGNMENT,
        },
        {
            ...
            'sequence': 32,
            'picking_zone_id': ctx.env.ref(
                '__setup__.picking_zone_materiel'
            ).id,
            'zetes_picking_type': RANGEMENT_ASSIGNMENT,
        },
        {
    ...
    ctx.env['stock.picking.type'].search([('name', '=', 'Pick')]).write(
        {'subcode': 'PICK', 'sequence': 59, 'color': color_pick}
    )
    ctx.env['stock.picking.type'].search(
        [('code', '=', 'mrp_operation')]
    ).write({'sequence': 89, 'color': color_mrp})
...
def create_fix_delivery_picking_type(ctx):
    ...
    """
    ptype = ctx.env.ref(
        '__setup__.stock_picking_type_fix_ship', raise_if_not_found=False
    )
...
def configure_procurement_rules(ctx):
    ...
    rulesBuy.write({'location_id': location_vlb.id})
...
def create_procurement_rules(ctx):
    ...
    for record in types:
        ...
        for t in types:
            ...
            create_or_update(
                ctx, 'procurement.rule', '{}_{}'.format(xmlid, t), record
            )
    ...
    types = [
        {
            ...
            'picking_type_id': ref('__setup__.stock_picking_type_materiel').id,
            'group_propagation_option': 'propagate',
            'mts_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_materiel_prod'
            ).id,
            'mto_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_materiel_mto'
            ).id,
        },
        {
            ...
            'group_propagation_option': 'propagate',
            'mts_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_ali_prod'
            ).id,
            'mto_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_ali_mto'
            ).id,
        },
        {
            ...
            'group_propagation_option': 'propagate',
            'mts_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_medoc_prod'
            ).id,
            'mto_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_medoc_mto'
            ).id,
        },
        {
            ...
            'group_propagation_option': 'propagate',
            'mts_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_froid_prod'
            ).id,
            'mto_rule_id': ctx.env.ref(
                '__setup__.procurement_rule_froid_mto'
            ).id,
        },
    ]
    for record in types:
    ...
    route_mto_values = {
        'pull_ids': [
            (
                ...
                0,
                [
                    ref('__setup__.procurement_rule_materiel_mto_mtu').id,
        ...
        '__setup__.stock_location_route_sale_bo',
        {
            ...
            'pull_ids': [
                (
                    ...
                    0,
                    [
                        ref('__setup__.procurement_rule_ali_sale').id,
...
def set_picking_zone(ctx):
    ...
    for (
        main_location_xmlid,
        picking_zone_xml_id,
    ) in main_locations_picking_zone_mapping.iteritems():
...
def set_product_expiry(ctx):
    ...
    ctx.env['product.category'].search([]).write(
        {'alert_time': 1, 'removal_time': 91, 'life_time': 121}
    )
