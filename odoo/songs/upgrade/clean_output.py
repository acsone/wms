...
def has_picking_noship_moves(ctx, picking):
    no_ship = (
        ctx.env.ref('__setup__.deliver_carrier_by_client').id,
...
def get_picking_stats(picking):
    ...
    states = [
        '%s: %d' % (state, len(val)) for (state, val) in states.iteritems()
    ]
    states.sort()
    return "%s\t%s" % (picking.name, ', '.join(states))


def transfer_picking(
    ctx, picking, date_done=None, change_pick_type=True, force=False
):
    fix_pick_type = ctx.env.ref('__setup__.stock_picking_type_fix_ship')
        ...
        if force:
            if pack.product_id.tracking != 'none':
                ...
                if packlot_qty < pack.product_qty:
                    unknown_lot = ctx.env['stock.production.lot'].search(
                        [
                    ...
                    if not unknown_lot:
                        unknown_lot = ctx.env['stock.production.lot'].create(
                            {
                                'product_id': pack.product_id.id,
                                'name': 'Unknown',
                            }
                    ...
                    pack.write(
                        {
                            'pack_lot_ids': [
                                (
                                    ...
                                    0,
                                    {
                                        'qty': pack.product_qty - packlot_qty,
        ...
        for line in pick.move_lines:
            ...
            for quant in line.quant_ids:
                if quant.qty == blocking.product_qty:
                    quant.history_ids = [(REMOVE, line.id), (ADD, blocking.id)]
                    break
...
def get_last_move(quant):
    return quant.history_ids.filtered(
        lambda rec: rec.location_dest_id == quant.location_id
    )
...
def correct_stolen_ships(ctx):
    ...
    waiting_moves = ctx.env['stock.move'].search(
        [('location_id', '=', output_loc.id), ('state', '=', 'waiting')]
    )
    ...
    no_back_moves = (
        ...
        .search(
            [
                ...
                (
                    'picking_id.partner_id.is_sale_back_order_accepted',
                    '=',
                    False,
                ),
            ]
        ...
        .filtered(
            lambda rec: rec.order_id.carrier_id
            not in (pickup_delivery, long_term_delivery)
        )
    )
        ...
        for anc in ancestors:
            ...
            for quant in quants:
                if quant.location_id == move.location_id:
                    ...
                    quant.write({'location_id': 16, 'reservation_id': move.id})
                    ...
                    last_move_pick_quants_in_output = last_move_pick.quant_ids.filtered(
                        lambda rec: rec.location_id.id
                        in (output_loc.id, fix_output_20191220.id)
                    )
                    if move.split_from == last_move:
                        ...
                        last_move.write(
                            {
                                'quant_ids': [
                                    # swap!
                                    (ADD, q.id, 0)
                                    for q in last_move_pick_quants_in_output
                                ]
                                + [(REMOVE, quant.id, 0)]
                            }
                        )
                        ...
                        quant.write(
                            {'location_id': 16, 'reservation_id': move.id}
                        )
                    elif last_move_pick.product_qty > sum(
                        last_move_pick.mapped('quant_ids.qty')
                    ):
                        ...
                        other_pick_moves = pick_pick.move_lines.filtered(
                            lambda rec: rec.id != last_move_pick.id
                            and rec.product_id == last_move_pick.product_id
                            and rec.move_dest_id.state == 'done'
                        )
                        free_quants = other_pick_moves.mapped(
                            'quant_ids'
                        ) - other_pick_moves.mapped('move_dest_id.quant_ids')
                        if sum(
                            free_quants.mapped('qty')
                        ) == last_move_pick.product_qty - sum(
                            last_move_pick.mapped('quant_ids.qty')
                        ):
                        ...
                        q.location_id.name == 'Customers'
                        for q in last_move_pick.quant_ids
                    ):
                        # Case 7: like case 2, but the substitution happened
                        # more than once.
                        quants_to_swap = find_quant_to_swap(
                            last_move, quant.qty
                        )
                        last_move.write(
                            {
    ...
    for pack in Pack.browse(ids):
        ...
        for move in wrong_shipping.move_lines:
            if move.name.startswith(
                EXTRA_MOVE_PREFIX
            ) and move.product_id in pack.mapped('quant_ids.product_id'):
                if (
                    (
                        move.procurement_id
        ...
        transfer_picking(
            ctx, expected_ship, date_done=wrong_shipping.date_done
        )
...
def fix_shipping_not_recorded(ctx):
    ...
    * picking was done, and the picked quant is still in Output, ship move
    ...
    to_fix_pickings = ctx.env['stock.picking'].browse(
        list(
            (no_backorder_picking_ids | waiting_picking_ids)
            - skipped_picking_ids
        )
    )
    ...
    to_fix_pickings.mapped('move_lines').filtered(
    ...
    for picking in to_fix_pickings.with_context(skip_pdf_gen=1):
        count += 1
        ctx.log_line(
            '* %02d/%02d %s' % (count, len(to_fix_pickings), picking.name)
        )
...
def find_ship_move(pick_move):
    ...
    if ship_move:
        while True:
            backorder = Move.search([('split_from', '=', ship_move.id)])
            if backorder:
                ship_move = backorder
            else:
                break
...
def check_quants_in_output(ctx):
    ...
    * some are attached to a ship move which is waiting: do nothing
    ...
    """
    no_ship = (
        ctx.env.ref('__setup__.deliver_carrier_by_client'),
    ...
    for q in quants:
        pick = q.history_ids.filtered(
            lambda r: r.location_dest_id == q.location_id
        ).sorted('date')
        if pick:
            pick = pick[-1]
...
def kill_waiting_shipments(ctx):
    ...
    for ship in ships:
        ...
        if len(pick) == 0:
            ...
            ship.picking_type_id = ctx.env.ref(
    ...
    for picking in pickings:
        ...
        picking.message_post(
            'This delivery was forced pushed. The lot numbers are probably incorrect.'
        )
            ...
            move = ctx.env['stock.move'].create(
                {
                    ...
                    'name': 'correction inventaire 20190210: %s'
...
def restore_quants_in_fix_sortie(ctx):
    ...
    assert fix_output_20191220.name == 'Fix Sortie 20181220'
    quants = ctx.env['stock.quant'].search(
        [('location_id', '=', fix_output_20191220.id), ('qty', '>', 0)]
    )
    ship_ids = []
    for q in quants:
        pick = q.history_ids.filtered(
            lambda r: r.location_dest_id == output_loc
        ).sorted('date')
        if pick:
            pick = pick[-1]
        ship = find_ship_move(pick)
        ...
        if ship.state not in ('done', 'cancel'):
            q.write({'location_id': output_loc.id, 'reservation_id': ship.id})
